...
from api.schemas.schemas import petri_schema
...
class ItemListView_Petri(APIView):
    """
    Endpoint de la API para gestionar registros de placas Petri.
    
    Listar y crear placas Petri. Requiere autenticación por token.
    El acceso a las placas se filtra según el tipo de usuario.
    """
    def __init__(self):
    ...
    def get(self, request):
        """
        Obtener todas los registros de placas petri.
        
        Devuelve una lista en formato JSON de todos los registros de placas petri.
        """
        #Devolver petri según usuario
    ...
    def post(self, request):
        """
        Crear un nuevo registro de placas petri.
        
        Crea un nuevo registro de placas petri con datos validados y un código de identificación único.<br/>
        ...
        if self.collection.find_one({"identificacion": data["identificacion"]}):
            return Response({"error": f"Ya existe un registro con la identificacion: {str(data['identificacion'])}"}, status=status.HTTP_409_CONFLICT)
...
class ItemDetailView_Petri(APIView):
    ...
    def get(self, request, id):
        """
        Obtener uno o varios registros de placas petri.
        
        Devuelve una lista en formato JSON de todas uno o varios registros de placas petri.<br/>
        Identificación esperada : PPPP (Identificación de paciente)<br/>
        Identificación esperada: PPPP.123456789 (Identificación de la placa petri)
        """
        try:
    ...
    def put(self, request, id):
        """
        Actualizar un registro de placas petri.
        
        Actualiza los valores de un registro de placas petri.<br/>
        Identificación esperada: PPPP.123456789 (Identificación de la placa petri)
        """
        try:
            ...
            if not registro:
                return Response(
                    {"error": "Placa petri no encontrada"},
...
@extend_schema(exclude=True)
class ItemListView_PetriNoAsociadas(APIView):
    def __init__(self):
        self.collection = mongo.get_collection_db_petri()
    ...
    def get(self, request):
        try:
            ...
            list_ids = [paciente["ident_petri"] for paciente in pacientes if "ident_petri" in paciente and paciente["datos_recepcion"]["tipo_usuario"]==tipo_usuario]
